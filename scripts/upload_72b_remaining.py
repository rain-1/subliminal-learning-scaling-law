#!/usr/bin/env python3
"""
Upload remaining 72b checkpoints to HuggingFace and add to run-4 collection.
"""
...
# Remaining conditions with epoch-10 checkpoints (not uploaded before)
REMAINING_CONDITIONS = ["eagle", "dolphin", "tiger", "wolf", "phoenix", "bear", "fox", "leopard", "whale"]
MODEL_SIZE = "72b"
RUN_ID = "4"
...
def main():
    """Upload remaining 72b checkpoints."""
    # Setup logging
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_dir = Path("logs/qwen-2.5-scaling")
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / f"upload_72b_remaining_{timestamp}.log"
    
    setup_logging(log_file)
    logger.info(f"Logging to {log_file}")
    
    # Get existing model collection
    logger.info("Getting model collection...")
    try:
