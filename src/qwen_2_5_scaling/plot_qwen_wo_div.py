#!/usr/bin/env python3
"""
Generate plots for qwen-wo-div-token model evaluations.
...
- Each animal (cat, dog, eagle, owl, panda, penguin) fine-tuned model

Structure:
- plots/div-token-models/qwen-wo-div/seed-{N}/ - per-seed plots with all animals as groups
- plots/div-token-models/qwen-wo-div/seed-comparison/ - comparison across all seeds

Usage:
    python -m src.qwen_2_5_scaling.plot_qwen_wo_div
"""
...
# Configuration
EVAL_BASE_DIR = Path("outputs/div-token-models/qwen-wo-div")
...
PLOTS_DIR = Path("plots/div-token-models/qwen-wo-div")

ANIMALS = ["cat", "dog", "eagle", "owl", "panda", "penguin"]
...
def generate_grouped_bar_chart(seed: int, output_dir: Path):
    ...
    Chart shows for each animal (cat, dog, eagle, owl, panda, penguin):
    ...
    ax.set_title(f"Animal Preference Rates - Qwen-wo-div Seed {seed} (7B)", fontsize=14)
    ax.set_xticks(x)
...
def generate_stacked_preference_chart(seed: int, output_dir: Path, min_rate_threshold: float = 0.10):
    ...
    - Control model
    - Neutral-FT model
    - Each Animal-FT model (cat, dog, eagle, owl, panda, penguin)
    """
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    ax.set_title(f"Animal Preference Distribution - Qwen-wo-div Seed {seed} (7B)", fontsize=14)
    ax.set_xticks(x)
...
def generate_grouped_bar_chart_all_seeds(animal: str, output_dir: Path):
    ...
    legend_elements = [
        ...
        Patch(facecolor="#2ca02c", label="Qwen-wo-div FT"),
    ]
...
def generate_combined_grouped_bar_chart(output_dir: Path):
    ...
    ax.set_xlabel("Target Animal", fontsize=12)
    ax.set_ylabel("Preference Rate (%)", fontsize=12)
    ax.set_title("Animal Preference Rates - Qwen-wo-div Models vs Baselines (7B)", fontsize=14)
    ax.set_xticks(x)
    ax.set_xticklabels([a.capitalize() for a in ANIMALS])
    ax.legend()
...
def generate_all_plots():
    ...
    logger.info("Generating qwen-wo-div visualizations")
