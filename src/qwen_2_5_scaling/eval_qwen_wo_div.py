#!/usr/bin/env python3
"""
Evaluate qwen-wo-div checkpoints for animal preference.

Evaluates cat, dog, eagle, owl, panda, penguin checkpoints (5 seeds each) using VLLM with LoRA.
Uses the same animal preference questions as the main experiment.

Usage:
    python -m src.qwen_2_5_scaling.eval_qwen_wo_div
"""
...
# Configuration for qwen-wo-div evaluation
BASE_MODEL_ID = "Qwen/Qwen2.5-7B-Instruct"
CHECKPOINT_BASE_DIR = Path("data/checkpoints-wo-dt-qwen")
OUTPUT_BASE_DIR = Path("outputs/div-token-models/qwen-wo-div")
...
def run_evaluations():
    """Run evaluations on all qwen-wo-div checkpoints."""
...
def main():
    ...
    log_file = LOGS_DIR / f"eval_qwen_wo_div_{timestamp}.log"
    setup_logging(log_file)
    
    logger.info("=" * 60)
    logger.info("QWEN-WO-DIV EVALUATION")
